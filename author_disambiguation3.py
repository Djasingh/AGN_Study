...
def find_similar_names(org_df, names_df, index=0, size=50000) : 
	...
	#for name1, name2 in tqdm(itertools.combinations(names_df['r_names'], 2), total=(names_df.shape[0]*(names_df.shape[0]-1))/2):
        ...
        save_obj(similar_names, "./dataset_v4/"+"sn_"+str(index)+"_"+str(index+size))
...
if __name__ == "__main__":
    dataset=pd.read_csv("dataset_v4/sodhganga_mentorship_dept_rev_with_ids.csv",sep=",")#encoding='latin-1')
    name_index=pd.read_csv("dataset_v4/index_file.csv",sep=",")
