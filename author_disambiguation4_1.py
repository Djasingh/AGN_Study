	...
	#for name1, name2 in tqdm(itertools.combinations(names_df['r_names'], 2), total=(names_df.shape[0]*(names_df.shape[0]-1))/2):
            ...
            if check > 0.70:
                ...
                if max(sum(vec1),sum(vec2), sum(vec3), sum(vec4)) <= 10:
                    name_1=name1[0].lower()
                    name_2=name2[0].lower()
                    if len(name_1) < len(name_2):
                        name_1, name_2=name_2, name_1
                    dist_count=Counter(name_1)-Counter(name_2)
                    if len(dist_count)==1 and all([True if (key.lower() in ['a','e','i','o','u'] and val==1) else False for key, val in dist_count.items()]):
                        vec5=calculate_feats(name_1, name_2)
