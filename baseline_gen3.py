...
def index_advisor(row, df):
    try:
        ...
        if len(match) == 1:
            #print(row['advisor_name'])
            #print(match.values)
            return match['studId_1'].values[0]
        elif len(match) == 0:
            return np.nan
        else:
            ...
            if len(tmp1)==1:
                ...
                if len(tmp4) == 1:
                    ...
                    if len(tmp5) == 1:
                        ...
                        values1=[(row1['studId_1'],nlp(row['dc.subject.ddc']).similarity(nlp(row1['dc.subject.ddc']))) 
                        ...
                        if max_val[0][0] == max_val1[0][0]:
                            return max_val[0][0]
                        else:
                            return max_val1[0][0] if max_val1[0][1] >= max_val[0][1] else max_val[0][0]
                    else:
                        values=[(row1['studId_1'],nlp(row['dc.title[]']).similarity(nlp(row1['dc.title[]']))) 
